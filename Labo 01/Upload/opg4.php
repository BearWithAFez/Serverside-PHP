...
// FIND FUNC
function findByName($companies, $name){
    $key = array_search($name, array_column($companies, 'name'));
    if ($key === FALSE) return;
    show($companies[$key]);
}

function findBySearchString($companies, $text){
    foreach($companies as $key => $value){
        //echo $value['name'];
        if(stripos($value['name'],$text) !== FALSE) show($companies[$key]);
    }
}
...
if($argv[1] == 'show'){
    if($argc < 3) return;
    $index  = $argv[2];
    if($index === NULL || $index < 0 || $index>count($companies )|| !is_numeric($index)) return;
    show($companies[$index]);
}
if($argv[1] == 'find'){
    if($argc < 3) return;
    $name  = $argv[2];
    if($name === NULL) return;
    findByName($companies,$name);
}
if($argv[1] == 'search'){
    if($argc < 3) return;
    $ss  = $argv[2];
    if($ss === NULL) return;
    findBySearchString($companies,$ss);
}
